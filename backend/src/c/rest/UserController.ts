...
export const UserController = {
  async AddUser(req: Request) {
    const { user } = req.body;
    if (!user) return controllerResult(null, false, new Error("User data absent in request body."));
    user.birthday = new Date(user.birthday);
    if (Number.isNaN(user.birthday.getDate())) return controllerResult(null, false, new Error("Invalid birthday."));
    const parseResult = userSchema.safeParse(user);
    if (!parseResult.success) return controllerResult(null, false, new Error("Invalid user data."));
